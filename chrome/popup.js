...
// Use chrome.storage instead of chrome.storage
...
document.getElementById("remove-api-key").addEventListener("click", () => {
   chrome.storage.sync.remove("apiKey").then(() => {
    ...
    document.getElementById("remove-api-key").style.display = "none";function showMessage(message, isSuccess) {
      const messageDiv = document.getElementById("message");
      if (!messageDiv) return;
      messageDiv.textContent = message;
      messageDiv.className = isSuccess ? "success" : "error";
        ...
        chrome.tabs.query({ active: true, currentWindow: true }).then((tabs) => {
          ...
          if (tab.url.startsWith("https://docs.google.com/forms/") || tab.url.startsWith("http://docs.google.com/forms/")) {
            ...
            chrome.scripting.executeScript({
              ...
              chrome.tabs.sendMessage(tab.id, { action: "solveMCQs", apiKey })
                .then(() => {
